...
public class ServerAPI {
    ...
    public static int register(SignalProtocolAddress address, IdentityKey identityKey, Map<Integer, ECPublicKey> preKeys, int signedPreKeyId, ECPublicKey signedPreKey, byte[] signedPreKeySignature) {
    ...
    public static List<SignalProtocolAddress> getContacts(String name) {
        synchronized (USER_DATA_LOCK) {
            return userData.entrySet().stream()
                    .filter(entry -> !entry.getKey().equalsIgnoreCase(name))
                    .map(entry -> new SignalProtocolAddress(entry.getKey(), entry.getValue().getDeviceId()))
