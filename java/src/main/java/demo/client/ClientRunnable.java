...
public class ClientRunnable implements Runnable {
    ...
    private InMemorySignalProtocolStore store;
    ...
    public void run() {
        ...
        List<SignalProtocolAddress> contacts = ServerAPI.getContacts(this.address.getName());;
    ...
    private void runSingleSession(SignalProtocolAddress contact) throws UntrustedIdentityException, InvalidKeyException, UnsupportedEncodingException, InvalidVersionException, InvalidMessageException, DuplicateMessageException, LegacyMessageException, InvalidKeyIdException, NoSessionException, InterruptedException {
        ...
        if (this.address.getDeviceId() < contact.getDeviceId()) {
            ...
            do {
                ...
                if (bundle.containsKey(contact.getName())) {
                    ...
                    PreKeySignalMessage incomingMessage = new PreKeySignalMessage(messages.get(0));
                    ...
                    for (byte[] message : messages) {
                        byte[] text = sessionCipher.decrypt(new SignalMessage(message));
        ...
        do {
            ...
            if (connected) {
                ...
                if (bundle.containsKey(contact.getName())) {
                    ...
                    for (byte[] message : messages) {
                        SignalMessage whisper = null;
                        try {
                            whisper = new SignalMessage(message);
    ...
    private void runGroupSession(List<SignalProtocolAddress> contacts) throws InvalidMessageException, LegacyMessageException, DuplicateMessageException, NoSessionException, InterruptedException {
        ...
        //Let's assume the lowest deviceID gets to initiate conversation
        if (this.address.getDeviceId() < contacts.stream().min(Comparator.comparing(SignalProtocolAddress::getDeviceId)).get().getDeviceId()) {
            ...
            for (SignalProtocolAddress contact : contacts) {
        ...
        do {
            ...
            for (Map.Entry<String, List<byte[]>> subbundle : bundle.entrySet()) {
                ...
                SignalProtocolAddress groupInitiator = contacts.stream().filter(v -> v.getName().equalsIgnoreCase(subbundle.getKey())).findFirst().get();
